...
def add_meta_data(filename: str, meta_data: Dict[str, Any]):
    ...
    if "large" in filename or "turbo" in filename:
        external_filename = os.path.basename(filename)
        external_filename = external_filename.split(".onnx")[0]
        print(external_filename)
        onnx.save(
            model,
            filename,
...
def main():
    ...
    if not os.path.exists(args.encoder):
        os.mkdir(args.encoder)
    if not os.path.exists(args.decoder):
        os.mkdir(args.decoder)
    ...
    encoder_filename = os.path.join(args.encoder, encoder_filename)
    torch.onnx.export(
        encoder,
        mel,
        encoder_filename,
    ...
    decoder_filename = os.path.join(args.decoder, decoder_filename)
    torch.onnx.export(
        decoder,
        (
            tokens,
    ...
    if "large" in args.model:
        decoder_external_filename = os.path.basename(decoder_filename)
        decoder_external_filename = decoder_external_filename.split(".onnx")[0]
    ...
    encoder_filename_int8 = os.path.join(args.encoder_int8, encoder_filename_int8)
    quantize_dynamic(
    ...
    decoder_filename_int8 = os.path.join(args.decoder_int8, decoder_filename_int8)
    quantize_dynamic(
